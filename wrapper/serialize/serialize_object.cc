...
// Local CHECK macro - throws on failure
#define CHECK(x)                                                                                                       \
  do {                                                                                                                 \
    if (!(x))                                                                                                          \
...
static std::array<PyObject*, intlistRange> intlistStorage;
std::array<PyObject*, intlistRange>& intlist = intlistStorage;
...
struct ObjMap {
  ...
  static std::tuple<Item*, size_t*> alloc(size_t n) {
    auto* items = std::allocator<Item>().allocate(n);
    size_t* indices;
    try {
      indices = std::allocator<size_t>().allocate(n);
    } catch (...) {
      std::allocator<Item>().deallocate(items, n);
      throw;
    }
  ...
  ~ObjMap() {
    std::allocator<Item>().deallocate(items, allocated);
    std::allocator<size_t>().deallocate(indices, allocated);
  }
  ...
  [[gnu::noinline]] [[gnu::cold]] void expand() {
    ...
    try {
      ...
      std::allocator<size_t>().deallocate(oldindices, oldallocated);
      std::allocator<Item>().deallocate(olditems, oldallocated);
      throw;
    }
    std::allocator<size_t>().deallocate(oldindices, oldallocated);
    std::allocator<Item>().deallocate(olditems, oldallocated);
...
std::vector<std::unique_ptr<ObjMap<PyObject*, size_t>>> offsetsFreelist;
std::vector<std::unique_ptr<ObjMap<size_t, PyObject*>>> reverseOffsetsFreelist;
...
struct SerializeObject : SerializeExpandable {
  ...
  SerializeObject() {
    ...
    offsets = std::move(offsetsFreelist.back());
    offsetsFreelist.pop_back();
  }
...
struct DeserializeObject : Deserialize<true> {
  ...
  DeserializeObject(std::string_view view) : Deserialize(view) {
    ...
    offsets = std::move(reverseOffsetsFreelist.back());
    reverseOffsetsFreelist.pop_back();
  }
...
struct ConstructorVector {
  static constexpr pyTypes type = pyTypes::list;
  static size_t size(const std::vector<py::object>& obj) {
    return obj.size();
  }
  static PyObject* get(std::vector<py::object>& obj, size_t index) {
...
struct CacheHash {
  size_t operator()(const std::pair<std::string_view, std::string_view>& v) const {
...
std::unordered_map<std::pair<std::string_view, std::string_view>, PyObject*, CacheHash> cachedGlobals;
...
__attribute__((visibility("default"))) void moodistSerializeInit(moodist::CoreApi* api) {
  ...
  api->deserializeObjectImpl = &moodistDeserializeObject;
}
}
