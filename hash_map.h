...
public:
  ...
  HashMap(const HashMap& n) {
    *this = n;
  }

  HashMap& operator=(const HashMap& n) {
    clear();
    for (auto& v : n) {
      insert(v);
    }
  ...
  template<typename T>
  T* allocate(size_t n) {
    using A = typename std::allocator_traits<Allocator>::template rebind_alloc<T>;
    T* retval = A().allocate(n);
    return retval;
  }
  template<typename T>
  ...
  iterator findSlowPath(size_t ki, KeyT&& key) const noexcept {
    ...
    auto equal = [](SecondaryItem* i, size_t ki, auto&& key) {
      ...
      return i->index == ki && Equal()(i->key, key);
    };
  ...
  auto insert(const std::pair<Key, Value>& x) {
